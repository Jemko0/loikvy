...
public class BookEditApplyPressedProcedure {
    public static void execute(Entity entity) {
        ...
        if (item.getItem() == Items.WRITTEN_BOOK) {
            ...
            if (bookContent != null) {
                WrittenBookContent newContent = getWrittenBookContent(newTitle, bookContent, newAuthor);
    ...
    private static @NotNull WrittenBookContent getWrittenBookContent(String newTitle, WrittenBookContent bookContent, String newAuthor) {
        String finalTitle = !newTitle.isEmpty() ? newTitle : bookContent.title().get(false);
        ...
        net.minecraft.server.network.Filterable<String> filterableTitle =
                net.minecraft.server.network.Filterable.passThrough(finalTitle);

        // Create new book content with updated title/author but same pages
